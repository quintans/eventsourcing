...
The solution for this challenge is to store events with a incremental key (monotonicity). With this we can then poll for events after the last polled event.
...
But there is catch. Records might not become available in the same order as the ID order.
...
> In PostgrSQL we can guarantee monotonicity if we use [Advisory Locks](https://www.postgresql.org/docs/9.6/explicit-locking.html) when inserting.
...
This is not saying that there aren't other solutions that would work better. Some database provide notification mechanisms from where we collect, like change streams from MongoDB.
...
The IDs generated by [xid](https://github.com/rs/xid) are ordered in time, with a millisecond precision, 
useful for databases where write operations don't happen in a single node. 
Since IDs need to be monotonic, it is important to use the query latency, mentioned above, to compensate server clock differences.

When we have databases where the write happens in one specific node, like PostgreSQL, if we guarantee monotonicity, we could use the bigserial hexadecimal representation for the eventID without the query latency. 
...
This is a simplistic and easy to implement approach but has the drawback of stressing the database due to the constant polling, as the number of read instances increase.
...
> To be honest, if we use a poller per write service I don't see how this would ever be a bottleneck, but again, we never know.
